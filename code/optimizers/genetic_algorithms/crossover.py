import copy
from optimizers.genetic_algorithms.helpers import repair_solution
...
def crossover(individual1, individual2, crossover="single table swap"):
    ...
    if crossover == "single table swap":
        ...
        # In these variables we will store the tables among parents that are the most similar but not equal
        ...
        for i, table1 in enumerate(individual1_set):
            for j, table2 in enumerate(individual2_set):
                ...
                but we also want similar tables so we don't have to repair as many duplicates
                and missing guests and aren't as destructive to our parent solutions'''
                if table1 != table2:
                    # here we use a set intersection to know what individuals they have in common
